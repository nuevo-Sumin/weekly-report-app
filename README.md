...
개발자는 주간 업무를 직접 입력하거나 CSV로 가져온 뒤 최종병합 텍스트를 작성해 제출하고, PL은 제출된 개발자 최종병합본 또는 제출 항목을 취합해 최종 주간업무보고 텍스트를 생성합니다.

## 목표

- 개발자가 금주/차주 업무 항목을 등록한다.
- 개발자가 CSV 업로드 그리드에서 필요한 행을 선택해 저장하고 병합한다.
- 선택 항목은 업무 구분과 단위업무 기준으로 병합된다.
- 개발자는 최종병합 텍스트를 수정한 뒤 PL에게 제출한다.
- PL은 개발자가 제출한 최종병합본과 제출 항목을 조회하고 최종 취합본을 작성한다.
...
- 금주/차주 업무 항목 등록
- 사업관리/수행 구분 등록
- CSV 업로드, 기존 완료건 제외 표시, row별 주차 선택
- CSV 업로드 그리드에서 필요한 행 선택 후 저장
- 수기 입력 임시저장, 저장
- 저장된 CSV/수기 입력 항목 기준 최종병합 텍스트 자동 생성
- 최종병합 텍스트 직접 수정, 저장, 복사
- 최종병합 텍스트 제출
- 사용자별/보고기간별 개발자 FINAL 제출본은 1개만 유지
- 팀장 취합 전에는 제출 취소 후 수정 및 재제출 가능
- FINAL 제출본에 포함되지 않은 현재 업무 항목 삭제

### PL 취합 흐름

- 제출된 개발자 업무 항목 조회
- 개발자가 제출한 최종병합본 조회
- 팀장 취합에 사용된 개발자 FINAL은 개발자가 취소하거나 수정할 수 없도록 보호
...
- `DELETE /api/report-items/{itemId}`
- `POST /api/report-items/submit`

### PL 취합

- `GET /api/admin/report-items`
- `GET /api/admin/merged-reports?reportStartDate=YYYY-MM-DD&reportEndDate=YYYY-MM-DD`
...
최근 확인된 백엔드 테스트는 37개입니다.

## 배포 방향

현재 앱은 Spring Boot 하나로 React 정적 파일과 API를 같이 제공할 수 있습니다.

현재 선택한 1차 배포 방향은 Oracle Cloud Always Free VM에 Spring Boot 단일 서버와 MySQL을 함께 운영하는 방식입니다.
...
현재 사용 패턴은 실사용자 약 10명, 주 1~2일, 주로 수요일 업무시간 사용, 텍스트/CSV 중심의 소량 데이터입니다. 이 조건에서는 상시 켜둘 저전력 서버가 없는 로컬 고성능 PC 운영보다 Oracle Cloud Always Free가 비용과 안정성 측면에서 유리합니다. 향후 보고서 rephrasing 기능이 필요하면 Oracle 서버가 내부 API로 사무실 PC의 Ollama/local LLM을 호출하는 하이브리드 구조를 검토합니다.
...
1. Oracle Cloud 배포 환경 구성
   - Oracle Cloud 계정/VM 생성
   - 방화벽 및 보안 목록 설정
   - Ubuntu 패키지, Java, MySQL 설치
   - `weekly-report.env` 운영 환경변수 작성
   - systemd 서비스 등록
   - GitHub 기준 배포/재배포 절차 확인

2. 운영 DB 초기화 및 권한 운영 정리
   - 운영 MySQL DB/사용자 생성
   - 최초 관리자 계정 생성
   - PL 권한 수동 승인 SQL 절차 확인
   - 백업/복구 스크립트 실제 실행 검증

3. 실사용 시나리오 E2E 점검
   - 개발자 회원가입/로그인
   - CSV 업로드, 기존 완료건 제외, row 선택 저장
   - 최종병합 저장/제출 취소/수정/재제출
   - PL 제출본 조회 및 최종 취합 저장/복사
   - FINAL이 아닌 항목 삭제, 팀장 취합 후 FINAL 취소 차단 확인
...
5. 운영 UX 마무리
   - 오류 메시지/빈 상태 문구 최종 점검
   - 관리자 화면 필터 사용성 확인
   - 모바일 대응은 최소 수준 유지, 데스크톱 사용성 우선

6. 이후 확장 후보
   - 제출 취소/반려/승인 워크플로우
   - CSV 원본 파일 보관 및 업로드 이력 테이블
   - 보고서 rephrasing용 local LLM 연동
   - 도메인/HTTPS 적용
